...
contract DltToken{
    ...
    function calculateFivePercent(uint256 _amount)  internal pure  returns(uint256 fivePercentage ) {
    ...
    function transfer(address _reciever, uint256 _amountOfToken) external  {
        ...
        uint256 fivePercentage = calculateFivePercent(_amountOfToken);

        totalSupply = totalSupply - fivePercentage;

        burn(msg.sender, fivePercentage);
